$(document).ready(function () {
    ...
    function courseRow(tableID, courseData, rowFunction, rowIndex = 1) {
        ...
        newRow.append(
            // Delete button
            $("<td>")
                .addClass("px-6 py-3 text-left border-b-2")
                .html(rowFunction)
        )
        table.append(newRow)
        ...
        try {
            ...
            if (response.data) {
                ...
                if (!studentCourses.includes(courseData.code)) {
                    ...
                    courseRow(studentCoursesTable, courseData, deleteButton, courseCounter)
    ...
    $("#generate-courses").click(async function () {
        ...
        possibleCourses = [...new Map(possibleCourses.map(v => [v['code'], v])).values()]
        ...
        availableCourses.forEach(course => courseRow(availableCoursesTable, course, addButton))
    })
    ...
    $(noPreReqTable).on("click", ".add", function () {
        ...
        //const courseCode = tableRow.find("td:first").text()
        //addCourseToTable($(this).courseCode)
        tableRow.remove()
    })
